...
- 作業開始時に現在のブランチと作業ツリーを確認してください。
- `main`または`develop`上では変更を開始せず、原則として最新の`develop`から作業内容ごとのブランチを作成してください。
- `main`、`develop`へ直接コミット・直接Pushしないでください。
- 依頼内容と現在の作業ブランチの目的が異なる場合は、別ブランチへ分けてください。
- 別の目的の変更や複数Issueの変更を、同じブランチやコミットへ混ぜないでください。
- Push前に、リモートの`develop`との差分、変更ファイル、コミット一覧、確認結果を確認してください。
- `git push --force`および`git push --force-with-lease`は、ユーザーから明示的な指示がない限り実行しないでください。
- ブランチ命名、コミット粒度、Push前確認、マージ方式の詳細は `docs/git-workflow.md` に従ってください。
...
このリポジトリでは、テキストファイルの改行コードをLFに統一してください。
新規作成・編集するテキストファイルはLFで保存してください。
改行コードの扱いは、ルートの`.gitattributes`と`biome.json`に従ってください。
